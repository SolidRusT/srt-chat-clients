...
pipe.to(device)
pipe.unet = DistributedDataParallel(pipe.unet, device_ids=[local_rank])
...
def generate_image(prompt, ckpt):
    ...
    pipe.unet.module.load_state_dict(load_file(hf_hub_download(repo, checkpoint), device=device))
...
# Gradio Interface
description = """
This demo utilizes the SDXL-Lightning model by ByteDance, which is a lightning-fast text-to-image generative model capable of producing high-quality images in 4
```
steps.
